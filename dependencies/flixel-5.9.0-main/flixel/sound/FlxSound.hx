...
class FlxSound extends FlxBasic
{
	...
	{
		// NovaFlare fix: destroy() 会把 _transform 置为 null，但 FlxG.sound.list 并不会移除这些
		// 已销毁的实例（SoundFrontEnd.destroy()），随后 list.recycle() 可能把它们回收复用，或者
		// Lua 侧仍持有引用再次写入 volume。此时解引用 _transform 会写入 NULL+0x30 触发
		// 原生 SIGSEGV（fault_address=0x30）。这里做防御性返回，且不重建 _transform，
		// 以免掩盖“对象已销毁”的状态。
		if (_transform == null)
			return;
