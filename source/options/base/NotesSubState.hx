...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		tip.setFormat(Paths.font(Language.get('fontName', 'main') + '.ttf'), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tip.borderSize = 2;
		add(tip);

		tipTxt = new FlxText(tipX, tipY + 24, 0, '', 16);
		tipTxt.setFormat(Paths.font(Language.get('fontName', 'main') + '.ttf'), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipTxt.borderSize = 2;
		add(tipTxt);
		updateTip();
	...
	override function update(elapsed:Float)
	{
		...
		if (generalPressed)
		{
			...
			else if (pointerY() >= hexTypeLine.y && pointerY() < hexTypeLine.y + hexTypeLine.height && Math.abs(pointerX() - 1000) <= 84)
			{
				...
				hexTypeNum = getHexCharAtPointer(pointerX());
				if (hexTypeNum > 5)
					hexTypeNum = 5;
	...
	{
		// 逐字渲染: 每字一个 FlxText, 用 letter.x - letter.offset.x 还原逻辑左边界
		if (hexTypeNum > 0)
		{
	...
	/** 根据点击位置算出十六进制文本的光标插入位置 */
	function getHexCharAtPointer(pointerX:Float):Int
	{
		var pos:Int = 0;
